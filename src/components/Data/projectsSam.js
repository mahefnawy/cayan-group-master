...
const projects = [
  ...
  const addCurrenciesToProjects = async (allProjects) => {
    ...
    fx.base = 'TRY'
      ...
      const returnValue = allProjects.map((project) => {

        console.log(fx.rates)

        project.priceMin.USD = Math.ceil(fx.convert(project.priceMin.TRY, {from: 'TRY', to: 'USD'}))
        project.priceMax.USD = Math.ceil(fx.convert(project.priceMax.TRY, {from: 'TRY', to: 'USD'}))
        project.types = project.types.map((type) => {
          type.minPrice.USD = Math.ceil(fx.convert(type.minPrice.TRY, {from: 'TRY', to: 'USD'}))
          // type.maxPrice.USD = Math.ceil(fx.convert(type.maxPrice.TRY, {from: 'TRY', to: 'USD'}))
          return type
        })
        return project
      })
...
export { projects, addCurrenciesToProjects } ;
